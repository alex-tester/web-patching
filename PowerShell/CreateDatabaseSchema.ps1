...
$sqlInstance = "SQL03"
$databaseName = "PatchingAutomation"
$databasePath = "C:\Program Files\Microsoft SQL Server\MSSQL14.MSSQLSERVER\MSSQL\Data" #no \ at end
$dbOwners = @("Observicing\GS-SQL-Admins") #Still need to implement RBAC


#Service account options
#Ensure DB Authentication is set to mixed if using a local account
$createLocalDbServiceAccount = $true
$LocalDbServiceAccountName = "patching_automation_db_service"
$LocalDbServiceAccountPassword = "Changeme123#"
...
$createLocalServiceAccountSql = Get-Content "$($sqlScriptPath)\DatabaseTemplate\CreateDbServiceAccount.txt"
...
#Create default patching source
$populatePatchSrcSql = "
INSERT INTO [{0}].[dbo].[PatchingSource] (CreatedBy,CreatedOn,ModifiedBy,ModifiedOn,PatchingSource)
VALUES ('ADMIN','2020-05-28','ADMIN','2020-05-28','Microsoft')" -f $databaseName
Invoke-SqlCmd -ServerInstance $sqlInstance -Query $populatePatchSrcSql



#create Local Db Service Account
if ($createLocalDbServiceAccount)
{
    $createLocalServiceAccountSql = $createLocalServiceAccountSql -f $databaseName, $LocalDbServiceAccountName, $LocalDbServiceAccountPassword
    Invoke-Sqlcmd -ServerInstance $sqlInstance -Query $createLocalServiceAccountSql
